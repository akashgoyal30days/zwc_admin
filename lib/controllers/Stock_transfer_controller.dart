...
class StockTransferController extends GetxController {
  ...
  GetAllStockTransferListModel? getallstocktransferlist;
  Getstocktransferdetailsbyidmodel? getstocktransfersdetailsbyid;
  ...
  Future<bool?> uploadstocktransferdata(
      List<Addstocktransferwastedata> wasteadddata) async {
    showloading = true;
    update();
    List quantityarray = [];
    List ratesarray = [];
    ...
    var response = await APIClient.post(URLS.uploadstocktransferdata, body: {
  ...
  Future<GetAllStockTransferListModel?> getstocktransferlist(
    ...
    var response = await APIClient.post(URLS.getallstocktransferslist,
        body: {"branch": branchid, "fromdate": fromdate, "todate": todate});
    var body = json.decode(response.body);
    if (response.statusCode == 200) {
      getallstocktransferlist = GetAllStockTransferListModel.fromJson(body);
      showloading = false;
      log(body.toString());
      update();
    }
    return getallstocktransferlist;
  }

  Future<Getstocktransferdetailsbyidmodel?> getstocktransferdetailsbyid({
    String? docid,
  }) async {
    update();
    String? branchid =
